...
package body Ocarina.Analyzer.AADL.Semantics is
   ...
   is
      ...
      Destination_Entity : Node_Id;
      Source_Kind        : Node_Kind;
      Destination_Kind   : Node_Kind;
   begin
      ...
      then
         ...
         Destination_Entity :=
            Corresponding_Entity (Component_Type_Identifier
               (Destination_Type));
         if Source_Entity = Destination_Type then
            --  b) source is an implementation of the destination
            return True;
         else
            --  c) source and destination are compatible
            --  A deeper semantic check is application specific
            Source_Kind := Kind (Source_Entity);
            Destination_Kind := Kind (Destination_Entity);
            if Source_Kind = Destination_Kind then
               return True;
            end if;
         end if;
      end if;
